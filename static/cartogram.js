(function(exports) {
  ...
  d3.cartogram = function() {
    ...
    function carto(topology, geometries) {
      ...
      var originalObjects = object(projectedArcs, {type: "GeometryCollection", geometries: geometries}).geometries;
     ...
     var originalAreas = originalObjects.map(path.area);
     var calcNewAreas = function() {
        var areas = objects.map(path.area);
         // var totalDesiredArea = d3.sum(areas.map(function(d,i) {
         //     return d * values[i];
         // })),
         totalArea = d3.sum(areas),
            ...
            meta = objects.map(function(o, j) {
              var area = Math.abs(areas[j]), // XXX: why do we have negative areas?
                  v = +values[j],
                // kgd hack for desired area we want to specify ratio of existing area independent of total area 
                  // desired = totalArea * v / totalValue,
                  desired = v * +originalAreas[j],
         ...
         // console.log('Total area of regions: ' + totalArea);
         return {meta:meta, error:sizeErrorsTot/sizeErrorsNum};
     };
        
     var i = 0;
      while (i++ < iterations) {
          var res = calcNewAreas(),
          meta = res.meta, sizeError = res.error,
    ...
    carto.feature = function(topology, geom) {
      return {
        ...
        geometry: {
          type: geom.type,
          // kgd 220714 coordinates: topojson.object(topology, geom).coordinates
